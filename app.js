...
function defaultLabelCameraGuideState(){
  return {
    ...
    pass1Complete:false,
    pass2Complete:false,
    passTransitionUntil:0,
    pass1Map:null,
    pass1MapWidth:0,
    pass1MapHeight:0,
    pass1PreviewStart:0,
    pass1PreviewEnd:0,
    pass2StartReference:null,
    pass2Travel:0,
    pass2MapX:0,
    pass2MapScore:0,
    pass2MapConfidence:0,
    pass2MatchedX:0,
    hqCoverageBins:new Array(LABEL_CAMERA_PASS_BINS).fill(0),
    peakScore:0,
    previousScore:0,
    peakNewRatio:0,
    scoreDropFrames:0,
    ...
    ambiguousFrames:0,
    rejectedMotionFrames:0,
    motionStepHistory:[],
    guideLocked:false,
    instruction:'Pass 1 maps the full label. Keep rotating after the first loop for Pass 2 detail capture and HQ gap filling.'
  };
}
...
function renderLabelCameraPreviewUi(){
  ...
  if(s.pass1Complete&&s.pass1MapWidth){
    const bins=s.hqCoverageBins||[],railY=c.height-10,railH=6,railW=availW;
    ctx.fillStyle='rgba(255,255,255,.10)';ctx.fillRect(pad,railY,railW,railH);
    const colorFor=v=>v<.25?'#ff3b4d':v<.45?'#ff8a2a':v<.65?'#ffd84a':v<.88?'#a9f542':'#28d86b';
    for(let i=0;i<bins.length;i++){
      const x0=pad+i/bins.length*railW,x1=pad+(i+1)/bins.length*railW;
      ctx.fillStyle=colorFor(clamp(bins[i]||0,0,1));ctx.fillRect(Math.floor(x0),railY,Math.max(1,Math.ceil(x1-x0)+1),railH);
    }
    if((s.scanPass||1)===2){
      const frameW=Math.max(1,s.previewAnchor?.canvas?.width||1),mapW=Math.max(1,s.pass1MapWidth),pos=(((Number(s.pass2MapX)||0)+frameW/2)%mapW+mapW)%mapW,mx=pad+(pos/mapW)*railW;
      ctx.save();ctx.strokeStyle='rgba(255,255,255,.98)';ctx.lineWidth=2;ctx.shadowColor='rgba(53,207,255,.9)';ctx.shadowBlur=5;ctx.beginPath();ctx.moveTo(mx,railY-4);ctx.lineTo(mx,railY+railH+2);ctx.stroke();ctx.restore();
    }
  }else{
...
function labelCameraTrackingFrameFromCanvas(source,template=null){
  if(!source?.width||!source?.height)return null;
  const aspect=template?.aspect||clamp(source.width/Math.max(1,source.height),.10,3.5);
  const narrow=template?.narrow??(aspect<.68),w=template?.w||Math.max(34,Math.min(108,Math.round(92*aspect))),h=template?.h||Math.max(28,Math.min(92,Math.round(w/aspect)));
  const gw=template?.gw||(narrow?72:64),gh=template?.gh||clamp(Math.round(gw/aspect),narrow?54:28,narrow?112:78);
  const preview=document.createElement('canvas');preview.width=w;preview.height=h;preview.getContext('2d',{alpha:false}).drawImage(source,0,0,w,h);
  const g=document.createElement('canvas');g.width=gw;g.height=gh;const gx=g.getContext('2d',{willReadFrequently:true,alpha:false});gx.drawImage(source,0,0,gw,gh);
  const rgba=gx.getImageData(0,0,gw,gh).data,gray=new Uint8Array(gw*gh),edge=new Uint8Array(gw*gh);
  for(let i=0,j=0;i<rgba.length;i+=4,j++)gray[j]=Math.round(.2126*rgba[i]+.7152*rgba[i+1]+.0722*rgba[i+2]);
  let edgeSum=0,edgeCount=0;
  for(let y=1;y<gh-1;y++)for(let x=1;x<gw-1;x++){
  ...
  return {canvas:preview,w,h,gw,gh,gray,edge,texture:edgeCount?edgeSum/(edgeCount*255):0,aspect,narrow};
}
async function labelCameraTrackingFrameFromCapture(capture,template=null){
  const c=await labelCameraCaptureToCanvas(capture,{maxEdge:720,mode:'core'});
  return labelCameraTrackingFrameFromCanvas(c,template);
}
...
function estimateLabelCameraMotion(a,b,lockedDirection=null){
  ...
  // Keep enough search range to recover from a missed frame, but do not blindly trust
  // the strongest mathematical match. Repeating ingredients / instructions can produce
  // several nearly-equal overlap solutions at very different horizontal positions.
  ...
  const top=[];
  const keepCandidate=c=>{
    let i=0;while(i<top.length&&top[i].weighted>=c.weighted)i++;
    top.splice(i,0,c);if(top.length>8)top.length=8;
  };
  for(const direction of dirs){
    for(let ow=maxOw;ow>=minOw;ow-=owStep){
      ...
      for(let dy=-dyMax;dy<=dyMax;dy+=2){
        ...
        keepCandidate({direction,overlap:ow,dy,score,weighted,newRatio:1-fraction});
      }
    }
  }
  const best=top[0];if(!best)return null;
  const separation=Math.max(4,Math.round(a.gw*.09));
  const second=top.find(c=>c!==best&&(c.direction!==best.direction||Math.abs(c.overlap-best.overlap)>=separation||Math.abs(c.dy-best.dy)>=4));
  const margin=second?best.weighted-second.weighted:1;
  const travelDifference=second?Math.abs(second.newRatio-best.newRatio):0;
  // Dense repeated text often yields two convincing but spatially incompatible answers.
  // In that situation uncertainty is safer than inventing panorama distance.
  const ambiguous=!!second&&margin<(aspect<.68?.018:.016)&&travelDifference>.075;
  return {...best,secondScore:second?.score??0,ambiguityMargin:margin,ambiguous};
}
...
function labelCameraZoneSimilarity(a,b,x0f,x1f){
  if(!a||!b||a.gw!==b.gw||a.gh!==b.gh)return 0;
  const gw=a.gw,gh=a.gh,x0=clamp(Math.floor(gw*x0f),0,gw-1),x1=clamp(Math.ceil(gw*x1f),x0+1,gw);
  let n=0,sa=0,sb=0,sea=0,seb=0;
  for(let y=1;y<gh-1;y+=2)for(let x=x0;x<x1;x+=2){const i=y*gw+x;sa+=a.gray[i];sb+=b.gray[i];sea+=a.edge?.[i]||0;seb+=b.edge?.[i]||0;n++;}
  if(n<8)return 0;
  const ma=sa/n,mb=sb/n,mea=sea/n,meb=seb/n;
  let cov=0,va=0,vb=0,ecov=0,eva=0,evb=0,raw=0;
  for(let y=1;y<gh-1;y+=2)for(let x=x0;x<x1;x+=2){const i=y*gw+x,da=a.gray[i]-ma,db=b.gray[i]-mb,dea=(a.edge?.[i]||0)-mea,deb=(b.edge?.[i]||0)-meb;cov+=da*db;va+=da*da;vb+=db*db;ecov+=dea*deb;eva+=dea*dea;evb+=deb*deb;raw+=Math.abs(a.gray[i]-b.gray[i]);}
...
function labelCameraLoopReturnEvidence(reference,frame){
  if(!reference||!frame)return {strong:false,near:false,score:0,corr:-1,identity:0,raw:0,zones:[0,0,0,0,0]};
  ...
  const zoned=labelCameraZonedEvidence(reference,frame);
  const narrow=!!frame.narrow;
  const supportStrong=corr>(narrow?.24:.28)||identity>(narrow?.75:.79)||raw>(narrow?.80:.83);
  const supportNear=corr>(narrow?.16:.19)||identity>(narrow?.69:.72)||raw>(narrow?.73:.76);
  // Loop identity comes from the whole frame: 10/20/40/20/10 zones must agree.
  // The old correlation/identity metrics are retained only as supporting evidence.
  const strong=zoned.strong&&supportStrong;
  const near=zoned.near&&supportNear;
  const support=Math.max(clamp((corr-.10)/.58,0,1),clamp((identity-.58)/.38,0,1),clamp((raw-.64)/.31,0,1));
  const score=clamp(zoned.score*.72+support*.28,0,1);
  return {strong,near,score,corr,identity,raw,zones:zoned.zones,zoneOuter:zoned.outer,zoneShoulders:zoned.shoulders,zoneCenter:zoned.center};
}
...
function labelCameraMarkCoverage(layout,weight=1){
  ...
  const frameW=Math.max(1,Number(layout.frameWidth)||Math.max(1,Number(layout.end)-Number(layout.start))||1);
  const mapW=Math.max(1,s.pass1MapWidth),center=((Number(layout.start)+frameW/2)%mapW+mapW)%mapW,radius=frameW*.50;
  for(let i=0;i<bins.length;i++){
    const x=(i+.5)/bins.length*mapW,linear=Math.abs(x-center),dist=Math.min(linear,mapW-linear);
...
function labelCameraRebuildCoverage(){
  const s=labelCameraGuideState;if(!s)return;
  s.hqCoverageBins=new Array(LABEL_CAMERA_PASS_BINS).fill(0);
  for(const cap of labelCameraCaptures){const layout=cap.mapLayout;if(layout)labelCameraMarkCoverage(layout,(cap.capturePass||1)===2?1:.86);}
}
function labelCameraHqCoverageRatio(){
  const bins=labelCameraGuideState?.hqCoverageBins||[];if(!bins.length)return 0;
  return bins.reduce((sum,v)=>sum+(v>=.42?1:clamp(v/.42,0,1)),0)/bins.length;
}
function labelCameraCoverageNeedAt(x,frameW){
  const s=labelCameraGuideState,bins=s?.hqCoverageBins;if(!s?.pass1MapWidth||!bins?.length)return true;
...
function labelCameraMapCandidateFrame(map,x,frame){
  ...
  const c=document.createElement('canvas');c.width=frame.canvas.width;c.height=frame.canvas.height;const cx=c.getContext('2d',{alpha:false});
  const first=Math.min(w,mapW-sx);cx.drawImage(map,sx,0,first,map.height,0,0,first,c.height);
  if(first<w)cx.drawImage(map,0,0,w-first,map.height,first,0,w-first,c.height);
  if(w<c.width)cx.drawImage(c,0,0,w,c.height,0,0,c.width,c.height);
  return labelCameraTrackingFrameFromCanvas(c,frame);
}
function labelCameraMatchPass1Map(frame){
  const s=labelCameraGuideState,map=s?.pass1Map;if(!s||!map||!frame)return null;
  const frameW=Math.max(1,frame.canvas.width),mapW=Math.max(1,map.width),expected=((s.pass2Travel||0)%mapW+mapW)%mapW;
  const span=Math.max(frameW*1.10,mapW*.09),step=Math.max(3,frameW*.12),candidates=[];
  for(let dx=-span;dx<=span;dx+=step){
    const x=((expected+dx)%mapW+mapW)%mapW,cand=labelCameraMapCandidateFrame(map,x,frame);if(!cand)continue;
    const z=labelCameraZonedEvidence(cand,frame),identity=labelCameraFrameIdentityScore(cand,frame),score=clamp(z.score*.78+identity*.22,0,1);
    candidates.push({score,x,zoned:z.score});
  }
  candidates.sort((a,b)=>b.score-a.score);const best=candidates[0]||{score:0,x:expected},separation=Math.max(frameW*.55,mapW*.035);
  const circularDist=(a,b)=>{const d=Math.abs(a-b);return Math.min(d,mapW-d);};
  const second=candidates.find(c=>circularDist(c.x,best.x)>=separation)||candidates[1]||{score:0,x:best.x};
  const margin=Math.max(0,best.score-second.score);
  const confidence=clamp(((best.score-.54)/.36)*.72+clamp(margin/.13,0,1)*.28,0,1);
  s.pass2MapScore=best.score;s.pass2MapConfidence=confidence;s.pass2MatchedX=best.x;
  s.pass2MapX=((best.x%mapW)+mapW)%mapW;return {...best,secondScore:second.score,confidence};
}
function beginLabelCameraSecondPass(frame,direction){
  const s=labelCameraGuideState;if(!s||s.scanPass!==1)return false;
  const snapshot=labelCameraSnapshotActivePreview();if(!snapshot)return false;
...
function labelCameraCurrentGlobalLayout(){
  const s=labelCameraGuideState,local=labelCameraCurrentPreviewLayout();if(!s||!local)return null;
  if((s.scanPass||1)===1||!s.pass1Complete)return {...local,pass:1};
  const frameW=Math.max(1,local.frameWidth||s.previewAnchor?.canvas?.width||1),mapW=Math.max(1,s.pass1MapWidth||frameW),raw=Number.isFinite(Number(s.pass2MapX))?Number(s.pass2MapX):(Number(s.pass2Travel)||0),start=((raw%mapW)+mapW)%mapW;
  return {start,end:start+frameW,frameWidth:frameW,direction:local.direction||s.previewDirection||null,pass:2,mapScore:s.pass2MapScore||0};
}
function appendLabelCameraLivePixels(frame,est){
  ...
  // v1.6.34: a nearly identical frame is NOT motion. Do not paint it into the
...
function advanceLabelCameraPreviewFromLive(){
  ...
  const identity=labelCameraFrameIdentityScore(s.previewAnchor,frame);
  if(!appended&&(est.rejectReason==='ambiguous'||est.rejectReason==='jump')){
    // Hold the existing panorama position while the text matcher is uncertain. If the
...
function updateLabelCameraUi(){
  ...
  if(!s.previewPanorama){help.textContent='Pass 1 · Alignment 0%';return;}
  const pct=Math.round(clamp(s.captureProgress||0,0,1)*100);
  if(s.pass2Complete||s.loopHint){help.textContent='Loop detected · Pass 2 complete · Ready to finish';help.classList.add('match-loop');return;}
  if((s.passTransitionUntil||0)>performance.now()){help.textContent='Loop detected · Pass 1 complete · Continue for Pass 2';help.classList.add('match-good');return;}
  if(labelCameraWorking&&labelCameraSessionCount>0){help.textContent=`Pass ${s.scanPass||1} · Alignment 100% · Capturing`;help.classList.add('match-good');return;}
  if((s.scanPass||1)===2){
    const coverage=Math.round(clamp(labelCameraHqCoverageRatio(),0,1)*100);
    const confidence=Math.round(clamp(s.pass2MapConfidence||0,0,1)*100);
    help.textContent=`Pass 2 · HQ ${coverage}% · Map ${confidence}% · Follow confidence rail`;
    if(confidence>=65)help.classList.add('match-good');else if(confidence>=40)help.classList.add('match-almost');
    return;
  }
  if(pct>=85){help.textContent=`Pass 1 · Alignment ${pct}% · Ready`;help.classList.add('match-good');return;}
  if(pct>=45){help.textContent=`Pass 1 · Alignment ${pct}%`;help.classList.add('match-almost');return;}
  help.textContent=`Pass 1 · Alignment ${pct}%`;
}
function drawLabelCameraOverlay(){
  ...
  // The ghost must be the EXACT PREVIOUS HQ content that was inside the blue
  // capture frame. It is shown outside the box as a frozen alignment reference.
  if(ghost){
    const edgeGap=0,slotH=bottom-top;
    const available=direction==='left'?(canvas.width-right-edgeGap):(left-edgeGap);
    const slotW=Math.max(14,available);
    const gx=direction==='left'?right+edgeGap:left-edgeGap-slotW,gy=top;
    const scale=Math.min(slotW/Math.max(1,ghost.width),slotH/Math.max(1,ghost.height));
    const drawW=Math.max(1,Math.round(ghost.width*scale));
    const drawH=Math.max(1,Math.round(ghost.height*scale));
    const drawX=direction==='left'?gx:gx+slotW-drawW;
    const drawY=gy+Math.round((slotH-drawH)/2);
    ctx.save();
    ctx.beginPath();ctx.rect(gx,gy,slotW,slotH);ctx.clip();
    ctx.globalAlpha=.56;
    ctx.drawImage(ghost,drawX,drawY,drawW,drawH);
    ctx.restore();
    ctx.save();
  ...
  // while only the middle 40% is treated as the high-value HQ detail slice.
  ...
  ctx.save();ctx.setLineDash([3,7]);ctx.strokeStyle='rgba(157,92,255,.13)';ctx.lineWidth=1;for(const f of [.10,.30,.70,.90]){const zx=left+(right-left)*f;ctx.beginPath();ctx.moveTo(zx,top);ctx.lineTo(zx,bottom);ctx.stroke();}ctx.setLineDash([]);ctx.restore();
...
function labelCameraLiveGhostPreview(side='right'){
  ...
  const core=geom.core;
  const targetH=240,scale=targetH/Math.max(1,core.sh),targetW=Math.max(96,Math.round(core.sw*scale));
  ...
  const ctx=out.getContext('2d');ctx.clearRect(0,0,out.width,out.height);ctx.drawImage(video,core.sx,core.sy,core.sw,core.sh,0,0,out.width,out.height);
  return out;
}
async function labelCameraGhostFromCapture(capture,side='right'){
  const meta=await labelCameraCaptureToCanvas(capture,{withMeta:true,maxEdge:900,mode:'core'});
  if(!meta?.canvas)return null;
  const out=document.createElement('canvas');
  const targetH=Math.min(480,Math.max(320,Math.round(meta.canvas.height)));const scale=targetH/Math.max(1,meta.canvas.height);
  out.width=Math.max(96,Math.round(meta.canvas.width*scale));out.height=targetH;
  out.getContext('2d').drawImage(meta.canvas,0,0,out.width,out.height);
  return out;
}
...
function tickLabelCamera(now=performance.now()){
  ...
  if(st?.previewPanorama&&!labelCameraWorking){
...
async function captureLabelCamera(fromAuto=false){
  ...
  const globalLayout=labelCameraCurrentGlobalLayout();
  ...
  // v1.6.35: never advance the ghost from a live frame. The old HQ ghost stays frozen
  // until a new HQ capture has been accepted and its own edge has been extracted.
  ...
  try{
    ...
    if(st&&hqTrackingFrame&&Array.isArray(st.loopStartSequence)&&st.loopStartSequence.length<3){
      const last=st.loopStartSequence[st.loopStartSequence.length-1];
      if(!last||labelCameraFrameIdentityScore(last,hqTrackingFrame)<.97)st.loopStartSequence.push(hqTrackingFrame);
    }
    ...
    if(labelCameraGuideState){
      ...
      if(record.mapLayout&&current.pass1Complete)labelCameraMarkCoverage(record.mapLayout,record.capturePass===2?1:.86);
      updateLabelCameraUi();drawLabelCameraOverlay();
    }
    toast(fromAuto?`HQ keyframe ${labelCameraSessionCount} saved · Pass ${record.capturePass}.`:`First HQ section captured. Rotate slowly and follow the ghosted edge.`);
...
function snapshotLabelCameraLiveReference(){
  const s=labelCameraGuideState;if(!s?.previewPanorama)return null;
  const pano=s.previewPanorama,direction=s.previewDirection||'right';
  let sx=Math.max(0,Math.floor(s.previewStartX||0)),ex=Math.min(pano.width,Math.ceil(s.previewEndX||pano.width));
  const loopComplete=!!s.loopHint&&Number.isFinite(s.loopCutX);
  if(loopComplete){
    if(direction==='left')sx=clamp(Math.round(s.loopCutX),0,Math.max(0,ex-1));
    else ex=clamp(Math.round(s.loopCutX),Math.min(pano.width,sx+1),pano.width);
  }
  const sw=Math.max(1,ex-sx);
  const c=document.createElement('canvas');c.width=sw;c.height=pano.height;c.getContext('2d').drawImage(pano,sx,0,sw,pano.height,0,0,sw,pano.height);
  return {canvas:c,start:sx,end:ex,frameWidth:s.previewAnchor?.canvas?.width||0,direction,loopComplete,loopScore:s.loopScore||0};
}
function labelCanvasStructure(canvas,w=72,h=72){
  if(!canvas?.width||!canvas?.height)return null;
  const aspect=canvas.width/canvas.height;h=Math.max(28,Math.round(w/Math.max(.12,aspect)));h=Math.min(110,h);
  const c=document.createElement('canvas');c.width=w;c.height=h;const cx=c.getContext('2d',{willReadFrequently:true});cx.drawImage(canvas,0,0,w,h);
  const d=cx.getImageData(0,0,w,h).data,gray=new Uint8Array(w*h),edge=new Uint8Array(w*h);let mean=0;
  for(let i=0,j=0;i<d.length;i+=4,j++){const g=Math.round(.2126*d[i]+.7152*d[i+1]+.0722*d[i+2]);gray[j]=g;mean+=g;}mean/=gray.length;
  let edgeSum=0,clip=0;
  for(let y=1;y<h-1;y++)for(let x=1;x<w-1;x++){const i=y*w+x,e=Math.min(255,Math.abs(gray[i+1]-gray[i-1])*.78+Math.abs(gray[i+w]-gray[i-w])*.22);edge[i]=e;edgeSum+=e;}
  for(const g of gray)if(g<8||g>247)clip++;
  return {w,h,gray,edge,mean,sharpness:edgeSum/(Math.max(1,(w-2)*(h-2))*255),clip:clip/gray.length};
}
function compareLabelCanvasStructure(aCanvas,bCanvas){
  const a=labelCanvasStructure(aCanvas),b=labelCanvasStructure(bCanvas);if(!a||!b)return {score:0,sharpness:0,clip:1};
  const w=Math.min(a.w,b.w),h=Math.min(a.h,b.h);let diff=0,edgeDiff=0,count=0;
  for(let y=1;y<h-1;y+=2)for(let x=1;x<w-1;x+=2){const ai=y*a.w+x,bi=y*b.w+x;diff+=Math.min(255,Math.abs((a.gray[ai]-a.mean)-(b.gray[bi]-b.mean)));edgeDiff+=Math.abs((a.edge[ai]||0)-(b.edge[bi]||0));count++;}
  const score=count?clamp(1-(diff*.52+edgeDiff*.48)/(count*255),0,1):0;
  return {score,sharpness:a.sharpness,referenceSharpness:b.sharpness,clip:a.clip};
}
async function labelCameraVerifyHqRecord(record,triggerFrame){
  if(!record||!triggerFrame?.canvas)return {ok:true,score:1};
  try{
    const hq=await labelCameraCaptureToCanvas(record,{maxEdge:640,mode:'core'});
    const cmp=compareLabelCanvasStructure(hq,triggerFrame.canvas);
    const blurRatio=cmp.referenceSharpness>0?cmp.sharpness/cmp.referenceSharpness:1;
    // Still-camera exposure can differ from the live preview, so structure matters more than brightness.
    // Reject only clear mismatches: a finger/hand entering, heavy motion blur, or a captured view that no longer matches the trigger frame.
    const ok=cmp.score>=.48&&blurRatio>=.48&&cmp.clip<.48;
    return {...cmp,blurRatio,ok};
  }catch(err){console.warn('HQ verification unavailable',err);return {ok:true,score:1};}
}
async function stitchLabelCameraCapturesFromPreview(captures,liveReference=null){
...
async function finalizeLabelCameraSession(){
  ...
  let captures=[...labelCameraCaptures];
  ...
  try{
    ...
    let out=await stitchLabelCameraCapturesFromPreview(captures,liveReference);
    if(!out){
      const pieces=[];
      for(let i=0;i<captures.length;i++){
